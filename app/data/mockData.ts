...
// 合并所有数据
export const mockData: MediaItem[] = [...mockMovies, ...mockTVShows];

// 获取所有类型
export const allGenres = Array.from(
  new Set(mockData.flatMap(item => item.genres))
).sort();

// 获取年份范围
export const yearRange = {
  min: Math.min(...mockData.map(item => item.year)),
  max: Math.max(...mockData.map(item => item.year))
};
